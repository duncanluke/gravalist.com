...
async function prepareAndSendReport() {
    ...
    const dailyFixedCosts = 1.55; // Vercel, Supabase, Domain prorated daily
    const estimatedProfit = estimatedRevenue - dailyFixedCosts;
    ...
    console.log('DAILY REPORT - Metrics Collected', {
        newSignups, newSales, estimatedRevenue, estimatedProfit, newRouteRegistrations, highestStep
    });
  ...
  <body>
    <div class="container">
      ...
      <div class="metric" style="background-color: #ecfdf5; border-radius: 6px; margin-top: 5px; border: 1px solid #f97316;">
        <span class="label" style="color: #c2410c;">Net Profit (After Server Costs)</span>
        <span class="value">$${estimatedProfit.toFixed(2)}</span>
      </div>
  ...
  Net Profit (After Server Costs): $${estimatedProfit.toFixed(2)}
  `;
