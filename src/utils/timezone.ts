...
export function formatTimezoneDisplay(timezone: string): string {
  const friendlyNames: { [key: string]: string } = {
    ...
    'Asia/Kolkata': 'Mumbai (India Standard Time)',
...
export function createRouteStartTime(eventDate: Date, timezone: string): Date {
  // Create 6:00 AM in the route's timezone
  const routeStart = new Date(eventDate);
  routeStart.setHours(6, 0, 0, 0);
...
export function getTimezoneOffset(timezone: string): string {
  try {
    ...
    const offset = (utc.getTime() - tz.getTime()) / (1000 * 60 * 60);
    const sign = offset >= 0 ? '+' : '-';
